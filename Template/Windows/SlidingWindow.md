...
def SlidingWindow(source,target):
    import collections
    need,window=collections.Counter(target),collections.Counter()
    left,right=0,0
    valid=0# the number of string which fill the need in windows.
    while(right<len(source)):
        ...
        while(windows need shrink):
            # answer usually update start here

            # answer usually update end here
            w_left=source[left]
            left+=1
            # windows data update start
